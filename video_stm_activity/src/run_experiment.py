...
def clean_caption(cap: str) -> str:
    ...
    # This works because your prompt is often multi-line and the model output usually comes after a blank line.
    if "\n\n" in cap:
        parts = [p.strip() for p in cap.split("\n\n") if p.strip()]
        cap = parts[-1] if parts else cap
...
def main():
    ...
    # Manual caption evaluation export (accuracy + hallucinations)
    parser.add_argument("--export_caption_eval", action="store_true",
                        help="Export a CSV template for manual caption accuracy + hallucination scoring.")
    parser.add_argument("--eval_videos", type=int, default=10,
                        help="How many videos to sample for manual caption evaluation export.")
    parser.add_argument("--eval_frames", type=int, default=5,
                        help="How many frames per video to include in manual caption evaluation export.")
    ...
    # Decide which videos to sample for manual caption eval export
    eval_items = items
    if args.export_caption_eval:
        rng = random.Random(seed)
        if len(items) > args.eval_videos:
            eval_items = rng.sample(items, args.eval_videos)
    ...
    # Manual evaluation rows (frame captions) — optional export
    combined_caption_eval_rows: List[Dict[str, Any]] = []
    ...
    for model_entry in vlm_models:
        ...
        for mode in modes:
            ...
            for video_path, true_label in items:
                ...
                for fr in frames:
                    ...
                    cap_time_s = time.perf_counter() - t0
                    caption_times_s.append(cap_time_s)
                ...
                if args.debug:
                    print("\n=== PER-FRAME CAPTIONS ===\n")
                    for t_s, cap in per_frame:
                        print(f"[{t_s:06.2f}s] {cap}")
                ...
                classifier_time_s = time.perf_counter() - cls_t0
                ...
                total_video_time_s = time.perf_counter() - video_t0
                total_caption_time_s = float(sum(caption_times_s)) if caption_times_s else 0.0
                avg_caption_time_s = (total_caption_time_s / len(caption_times_s)) if caption_times_s else 0.0
                n_frames = len(caption_times_s)
                ...
                per_video_rows.append({
                    ...
                    # Timing metrics (new)
                    "n_frames": n_frames,
                    "total_caption_time_s": round(total_caption_time_s, 6),
                    "avg_caption_time_s": round(avg_caption_time_s, 6),
                    "classifier_time_s": round(classifier_time_s, 6),
                    "total_video_time_s": round(total_video_time_s, 6),
                })
                ...
                # Optional: export a manual caption evaluation template (accuracy + hallucination rubric)
                if args.export_caption_eval:
                    # Only export for sampled videos (eval_items)
                    is_in_eval = any(os.path.basename(video_path) == os.path.basename(p) for (p, _) in eval_items)
                    if is_in_eval:
                        # choose evenly spaced frames (up to eval_frames)
                        if len(per_frame) <= args.eval_frames:
                            chosen = per_frame
                        else:
                            idxs = [int(i * (len(per_frame) - 1) / (args.eval_frames - 1)) for i in range(args.eval_frames)]
                            chosen = [per_frame[i] for i in idxs]

                        for t_s, cap_txt in chosen:
                            combined_caption_eval_rows.append({
                                "vlm_model": model_name,
                                "mode": mode,
                                "video": vid_name,
                                "frame_t": round(float(t_s), 3),
                                "caption": cap_txt,
        ...
        # ✅ Make sure folder exists right before writing
        ensure_dir(os.path.dirname(per_video_csv))
        ensure_dir(os.path.dirname(summary_csv))
        ensure_dir(os.path.dirname(stats_txt))

        if per_video_rows:
            ensure_dir(os.path.dirname(combined_per_video_csv))
            ensure_dir(os.path.dirname(combined_summary_csv))
    ...
    # Write manual caption evaluation template (optional)
    if args.export_caption_eval and combined_caption_eval_rows:
        eval_csv = os.path.join(outputs_dir, "caption_eval_template.csv")
        with open(eval_csv, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=list(combined_caption_eval_rows[0].keys()))
            w.writeheader()
            w.writerows(combined_caption_eval_rows)
        print(f"Saved manual caption eval template: {eval_csv}")
